...
BigUnsignedInABase::BigUnsignedInABase(const Engine::Containers::CString &s, Base base) {
	// Check the base.
	if (base > 36)
		throw "BigUnsignedInABase(std::string, Base): The default string conversion routines use the symbol set 0-9, A-Z and therefore support only up to base 36.  You tried a conversion with a base over 36; write your own string conversion routine.";
	// Save the base.
...
BigUnsignedInABase::operator std::string() const {
	...
	char *s = (char*)Engine::External::GetExternalAllocator()->Alloc(len + 1);
	s[len] = '\0';
	Index digitNum, symbolNumInString;
	...
	Engine::External::GetExternalAllocator()->Free(&s);
	return s2;
}

BigUnsignedInABase::operator Engine::Containers::CString() const {
	if (base > 36)
		throw "BigUnsignedInABase ==> std::string: The default string conversion routines use the symbol set 0-9, A-Z and therefore support only up to base 36.  You tried a conversion with a base over 36; write your own string conversion routine.";
	if (len == 0)
		return Engine::Containers::CString("0");
	// Some compilers don't have push_back, so use a char * buffer instead.
	char *s = (char*)Engine::External::GetExternalAllocator()->Alloc(len + 1);
	s[len] = '\0';
	Index digitNum, symbolNumInString;
	...
	Engine::Containers::CString s2(s);
