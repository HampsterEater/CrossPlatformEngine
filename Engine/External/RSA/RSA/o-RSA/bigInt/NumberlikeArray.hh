...
public:
	...
	~NumberlikeArray() {
		...
		if (blk != NULL)
		{
			for (u32 i = 0; i < len; i++)
				blk[i].~Blk();
			Engine::External::GetExternalAllocator()->Free(&blk);
		}
...
void NumberlikeArray<Blk>::allocate(Index c) {
	...
	if (c > cap) {
		...
		cap = c;
		blk = (Blk*)Engine::External::GetExternalAllocator()->Alloc(sizeof(Blk) * cap);
	}
}
...
void NumberlikeArray<Blk>::allocateAndCopy(Index c) {
	...
	if (c > cap) {
		...
		for (u32 i = 0; i < len; i++)
			oldBlk[i].~Blk();
		Engine::External::GetExternalAllocator()->Free(&oldBlk);
	}
}
