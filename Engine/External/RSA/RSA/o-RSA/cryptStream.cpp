...
void cryptStream::encrypt(Bunt mod, Bunt pub){
	...
	char* block = (char*)Engine::External::GetExternalAllocator()->Alloc(bSize*bufferSize);
	ifstream ifile;
	ofstream ofile;
	...
	char* temp = (char*)Engine::External::GetExternalAllocator()->Alloc(bufferSize);//new char[bufferSize];
...
void cryptStream::decrypt(Bunt mod, Bunt pri){
	...
	unsigned long bSize = calcBytes(mod).toUnsignedLong();
	char* block = (char*)Engine::External::GetExternalAllocator()->Alloc(bSize);//new char[bSize];
	ifstream ifile;
	ofstream ofile;
